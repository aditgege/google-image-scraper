...
from harvest_urls import HarvestURLs
...
class ScrapeImage:
    ...
    @staticmethod
    def get_keywords(keywords_file='search_keywords.txt'):
    ...
    def download_from_site(self, keyword, site_code):
        ...
        try:
            collect = HarvestURLs(no_gui=self.no_gui)
        except Exception as e:
        ...
        try:
            ...
            if site_code == Sites.GOOGLE:
                ...
                print('Invalid or failed to collecting URL links!')
                links = []
    ...
    def do_crawling(self):
        ...
        for keyword in keywords:
            ...
            if os.path.exists(os.path.join(os.getcwd(), dir_name)) and self.skip:
                print('Skip existing directory {}'.format(dir_name))
                continue
    ...
    def imbalance_check(self):
        ...
        if len(dict_too_small) >= 1:
            print('Imbalance file count directories:')
            for dir, n_files in dict_too_small.items():
                print('dir: {}, file_count: {}'.format(dir, n_files))
            print("Remove directories? (y/n)")
            answer = input()

            if answer == 'y':
                print("Removing imbalance file count directories...")
                for dir, n_files in dict_too_small.items():
                    shutil.rmtree(dir)
                    print('Removed {}'.format(dir))
                print('Run this program to re-download removed files. (with skip_already_exist=True)')
        else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--skip', 
                        type=str, 
                        default='true',
                        help='Skips already downloaded (bool)'
                             ' (default: true)')
                        
    parser.add_argument('--threads', 
                        type=int, 
                        default=4, 
                        help='Maximum threads of download (num)'
                             ' (default: 4)')
                        
    parser.add_argument('--google', 
                        type=str, 
                        default='true', 
                        help='Download from google.com (bool)'
                             ' (default: true)')
                        
    parser.add_argument('--full', 
                        type=str, 
                        default='false', 
                        help='Download full resolution (bool)'
                             ' (default: false)')
                        
    parser.add_argument('--face', 
                        type=str, 
                        default='false', 
                        help='Enable human face search (bool)'
                             ' (default: false)')
                        
    parser.add_argument('--no-gui', 
                        type=str, 
                        default='auto', 
                        help='Speed up full-res downloads (bool)'
                             ' (default: auto=false)')
                             
    parser.add_argument('--limit', 
                        type=int, 
                        default=0, 
                        help='Maximum download per thread'
                             ' (default: 0=inifinite)')
    ...
    print('Current Settings: skip:{}, threads:{}, google:{}, full_resolution:{}, face:{}, no_gui:{}, limit:{}'
          .format(_skip, _threads, _google, _full, _face, _no_gui, _limit))

    crawler = ScrapeImage(skip_already_exist=_skip, n_threads=_threads,
